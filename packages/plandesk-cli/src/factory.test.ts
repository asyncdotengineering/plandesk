...
describe('runFactoryInit', () => {
   ...
   * They must aim it at the same place, or command order silently decides
   * which copy of the skill a harness loads.
   */
  it('points the claude skill link at the same source connect uses', () => {
    const repo = makeTempDir('plandesk-factory-order-');
    runFactoryInit({ repoDir: repo });

    const link = join(repo, '.claude', 'skills', 'plandesk', 'SKILL.md');
    expect(readlinkSync(link)).toBe(SKILL_SYMLINK_TARGET);
    expect(readFileSync(link, 'utf8')).toBe(buildSkillMarkdown());
  });
