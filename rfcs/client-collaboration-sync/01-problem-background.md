...
- **Lossless projection basis exists.** `plandesk export`/`import` (`plandesk-export-v1`) already walks a project into a serializable shape; the `ClientView` projection is a *filtered, allow-listed* descendant of that work, not a greenfield serializer.
...
- **Live public API on the owner's instance.** Truly live, but forces the local-first machine to accept **inbound** public connections (tunnel / always-on box). Laptop closes → client sees nothing. Worse, filtering internal data *on read* at a public endpoint means one projection bug leaks real data. Rejected: breaks local-first reachability and is the weaker security posture.
