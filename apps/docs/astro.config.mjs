    ...
    starlight({
      ...
      plugins: [
        starlightLlmsTxt({
          ...
          description:
            'Plan Desk is a local-first, self-hostable planning workspace: a flow canvas of task nodes with labeled dependency edges, specs attached to nodes, a board, and an MCP server (29 tools) that coding agents use to scaffold plans, pick the next task, and update status live.',
          details:
